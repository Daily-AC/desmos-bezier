...
- **第十四期(2026-07-17,已验收):[A] 自动最优参数模式(默认开)**。用户诉求"每次帧渲染系统调成最优设置,感觉现在远远到不了渲染极限"。
  1. 参数校准(主会话,七素材真浏览器实验):默认阈值 60 在自然素材模糊后梯度分布的 p54–p76;容差 2.0→1.0 曲线数普涨 60–80%、8000 上限内无截断、耗时不变——这是最大安全增益。公式定案:阈值=非零梯度 p70 分位 clamp[30,120]、容差锁 1.0、上限 8000、精细按"有色内容占比≥5%"自动开(饱和度≥0.2 且非近黑的采样像素占比;最初用全图饱和度中位数,被白底狐狸图当场证伪——白底占多数把中位数拉到 0,主会话直改为占比规则)。
  2. 实现 codex(job: ~/.codex-jobs/desmos-bezier/20260717-auto-optimal):纯函数 autoRenderParams/lockAutoRenderAnalyses(node 可测,test_auto.js 17 断言);视频在 Pass A 色板采样帧逐帧分析取中位数全片锁定(防帧间参数闪烁);UI `[A] 自动` toggle 默认开,开启时三滑杆+精细禁用置灰但同步显示自动值,关闭恢复用户手动值;auto=false 路径与旧行为逐位一致(狐狸 182/1026/83 复核)。
  3. 主会话独立验收 21/21:狐狸自动 1535 曲线/340 精细填色(手动 1026/83);游戏帧(zmxy 720p)自动 3119 曲线/1999 填色,火焰橙黄渐变、伤害数字、石像纹理均可辨——低阈值捕获火焰内部边缘后色块分细,第十三期遗留的"火焰变红团"问题顺带解决;视频全片参数锁定(播放中 53 恒定);dragon-v2(近单色线稿)自动保持标准填色不加噪;关自动重渲染回基线;console 双路径零错误。
  4. 已知限制:①精细填色 maxPolygons 2000 在复杂游戏帧会截断(stats 有 '[!] 填色已截断' 提示),暂不放开——表达式总量与暂停延迟的取舍待观察;②自动模式增加每素材一次(视频≤12次)灰度+模糊+Sobel 分析,实测无感。
  5. 渲染极限结论(答用户问):贝塞尔拟合无精度极限(容差→0 逐点复刻折线),真实天花板=①边缘检测信息损失(自动阈值管这层)②平面填色表现不了渐变光晕(媒介固有)③Desmos 表达式承载(8000 内可用)。

- **第十三期(2026-07-17,已验收):视频/GIF 高清化(处理分辨率对齐图片路径)+ 流式预处理**。用户报"视频精细度远差于单帧图片"。
  1. 审计根因(主会话,同帧控制变量实证):两路径管线参数完全相同,唯一差异是输入分辨率——图片 maxDim 1000(小图高质量上采样 900),视频/GIF 一律 600 无上采样(`scaledVideoSize`)。720p 同帧 600px vs 1000px = 737 vs 1641 曲线(2.2×)、335 vs 590 填色。600 是第十期的内存预算(120 帧解码 ImageData 全量驻留 ~97MB,1000px 会到 ~270MB)。
  2. 甲方拍板:不损失高清,预处理时间可以变长。方案:尺寸规则统一到图片路径三段规则(新纯函数 `scaledMediaSize`,>1000→1000 / <500→高质量上采样 900 / 之间原样) + 预处理改流式两遍控内存(Pass A ≤12 帧色板分析后释放像素,Pass B 逐帧解码→矢量化→缩略图→丢像素,任意时刻解码帧驻留 ≤13 帧)。GIF 色板轮后重建 ImageDecoder,处理轮按 frameIndex 随机解码。进度文案:'[+] 分析色板… i/n' + '[+] 处理视频… i/n'。
  3. 实现 codex(job: ~/.codex-jobs/desmos-bezier/20260717-video-hd,effort=high):含 `handleImageFile` 改用共享尺寸函数(对旧算法 10000 组输入等价断言)、test_video.js 新增 41 断言(尺寸边界/流式结构/色板上限)、test_video_integration.js 基线重算(moving-shapes 三 asset 480×320→900×600,曲线数 2→8/8/5,新签名见 codex 汇报)。
  4. 主会话独立验收(node+playwright-core,--enable-precise-memory-info/--expose-gc):同帧精细度对齐 video 1591 vs image 1641 曲线(97%)、581 vs 590 填色;720p→1000px、480×320 GIF→900px 上采样均确认;120 帧 720p 预处理 21.2s(约 2×,甲方已接受),峰值堆 394MB 但强制 GC 后稳态驻留仅 +148MB(缩略图+矢量数据,证明像素未全量驻留,峰值是管线临时数组 GC 滞后);暂停提交 2567 表达式实测 337ms;GIF 播放推进正常;狐狸图基线 182/1026/83 逐位一致;test_fourier 22 + test_video 41 + test_video_integration 19 全绿;双路径 console 零错误。
  5. 验收插曲:Desmos CDN 一次 TLS 抖动打断首轮验收脚本(route.fetch 未兜底),重跑通过;route 注入 wrapper 抓 calculator 实例的方法比 addInitScript defineProperty 可靠(后者实测抓不到,原因未深究)。

- **第十二期(2026-07-16,已验收):预览面板最小化+拖动 + 移动端适配**。实现 codex(job: ~/.codex-jobs/desmos-bezier/20260716-panel-drag-mobile,effort=high;沙箱禁网禁端口,浏览器验证全归主会话),+182/-23 行,仅 UI 层,管线与表达式生成未动。
  1. 预览面板:顶部新增 `#previewHeader`("预览" + `[-]/[+]` 最小化按钮,previewCaption 同族样式)。整栏 Pointer Events 拖动(setPointerCapture,touch-action:none),拖动时切 inline left/top 并 clamp 在 #main 内,window resize + ResizeObserver 双路重 clamp;`.large` 降级模式 header 隐藏、`clearPreviewPosition()` 清 inline 定位防覆盖 inset:0。最小化=隐藏 header 以下全部段落,与图 2 折叠态互不干扰。
  2. 移动端:body 100dvh;≤720px 断点下五个滑杆包进 `#paramsDrawer`(桌面 `display:contents` 保证零差异,DOM 包装不影响布局),`[=] 参数` 按钮控制抽屉开合(默认收起,展开时滑杆整行 flex:1);hint 隐藏、stats 独占一行、面板宽 min(220px,48vw);`pointer:coarse` 下按钮 min-height 44px + touch-action:manipulation。
  3. **过程中暴露并修复一个 HEAD 就存在的潜伏 bug**(主会话直改):`#toolbar label { display:flex }`(author origin)一直压过 UA 的 `[hidden]{display:none}`,容差/谐波数互斥显隐在 HEAD 上实际失效(两者同时可见,empirical 确认;第八期验收时点未知何故未暴露)。修复:补 `#toolbar label[hidden] { display:none }`——与既有 `#videoControls[hidden]` 同一防御模式。
  4. 主会话独立验收(暗题 30 项,node+playwright-core 直驱,脚本会话 scratchpad 内、逻辑以本条为准):桌面 1280×800 dpr=2(最小化/恢复/拖动跟手无双倍偏移/四边 clamp/拖后重渲染位置保持/窗口缩小回收);.large 降级全屏且 header 不可见(含先拖后降级路径);移动 390×844 触摸(CDP dispatchTouchEvent 拖动/抽屉开合/无横向溢出/全链路渲染);桌面零回归(参数按钮隐藏、滑杆常显);容差/谐波互斥修复后双端验证;狐狸图 182 路径/1026 曲线/83 填色与 git HEAD 基线逐位一致;test_fourier.js 22 断言全绿;console 双端零新增错误。
  5. 已知取舍:移动端渲染后浮动面板会盖住 Desmos 底部表达式列表——本次功能(可拖/可最小化)即其解法,不另做避让。赛后继续打造的第一个功能。架构(甲方拍板):解码 → 逐帧跑共享矢量化管线(预处理,≤12fps/≤120帧/帧maxDim 600) → 覆盖层 canvas 播放矢量动画 → **暂停即方程**(暂停时当前帧一次 setState 提交 Desmos,播放时全部 hidden)。实现 codex(新 codex-dispatch 直驱链路,job: ~/.codex-jobs/desmos-bezier/20260713-video-input,方案确认一停 + GIF bug 修复一停,全程 ~50 分钟)。要点:
...
- **对外发布(2026-07-11 凌晨)**:演示视频已推站点 https://desmos-bezier.lab.qmledmq.cn:8443/demo-video.mp4 (200/11MB);项目已建 GitHub 私有仓库 github.com/Daily-AC/desmos-bezier(main,含 README/全部测试资产/视频,.impeccable 已 ignore),已按甲方拍板转 **public**(提交公链需要,IP 素材风险已提示、甲方知情),README 升级为评委版(效果图 docs/ 四张、管线图、六条技术决策、验收纪律、诚实披露、开发方法),对 AI 评委声明 progress.md 为机器可读项目史。
...
- **第十一期(2026-07-15 晚,主会话):造梦西游3 抖音演示视频**。成片桌面 `zmxy3-desmos-douyin.mp4`(40.6s/1080×1920 竖版/30fps/AAC 配乐),结构:标题卡 → 傅里叶画悟空(本轮系圆圈可见) → 贝塞尔+精细上色悟空(945 填色) → 后羿/七仙女/小龙女三连快切(4× 速) → 视频模式(B站扒的造梦西游3火魔斩实机 10s 段,600px/120帧,播放段后期裁切放大到全屏、暂停出方程、拖帧) → "一切皆方程"结尾卡。素材:百度百科 bkimg CDN(500×500 官方 Q 版 boss 立绘,拼 pic hash 直连可下)+ 4399 zmxy5 图集(newsimg.5054399.com 带 Referer 可下,悟空/小龙女/唐僧高清立绘)+ B 站(yt-dlp 需 `--cookies-from-browser chrome` 绕 412,无 cookie 必 412)。素材与工程文件在 `zmxy-assets/`(含 IP 内容,已入 .gitignore,勿提交公开仓)。制作链复用第十期:playwright recordVideo 分段录制(每段独立 context 免时间轴串扰)+ HTML 卡片/字幕截图 + ffmpeg concat(注意 concat 各路必须 setsar=1)+ Apple Loops 分层配乐。剪辑点按画面内容定位(录像时间轴与墙钟不一致,老坑),曾因 2s 粒度拼帧图误判把后羿/七仙女在上色落地前切走,重剪修复。重录脚本 scratchpad/record-zmxy.js、总装 scratchpad/assemble.sh(本期 scratchpad 会话结束即失效,脚本逻辑以本条记录为准)。

- **第十一期补丁(2026-07-16 凌晨,主会话):v2 双版本 + 两个用户反馈修复**。成片:桌面 `zmxy3-desmos-douyin.mp4`(竖版 v2,41s)+ `zmxy3-desmos-land.mp4`(横版,40.4s),均已发飞书。修复:①"悟空画一半重画三次"——根因有二:v1 剪辑把傅里叶段和贝塞尔段(同图两次绘制)拼在一起,且录制时在首遍自动绘制未完成时点了 [>] 重播(重播打断重启);v2 改为只用渲染触发的首遍自动绘制,单次连续。②视频模式发蒙——录制时没开精细开关,v2 开精细后 32 色共享调色板,画面清晰。**过程中发现的 app 疑似 bug(未修,待定夺):[>] 绘制重播在最终 commit(全量显形)之后点击是"隐形"的——overlay 动画画在已显形的 Desmos 图上,肉眼只见单帧闪烁;只有在首遍动画未提交前点击才有可见重播效果。若要修:startDrawAnimation 需先把表达式批量 hidden 再播。**制作方法沉淀:静止等待期会被 playwright 录像时间轴压缩,墙钟推算切点不可靠,本期用图纸区 YAVG 逐帧扫描(空白水平/跳变检测,find-blank.py / find-clear.py,在 zmxy-assets/)+ fps=2 拼帧条带目检定位所有切点。横版视频模式画质意外地比竖版更出效果(图纸区 16:9 与视频源同比,矢量画面近乎满幅)。
